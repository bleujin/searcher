...
public abstract class AbDocumentStream<T extends AbDocument, RT> implements Iterable<T> { 

	private Stream<T> stream;

	protected AbDocumentStream(Stream<T> stream) {
		this.stream = stream;
	}
	...
	public RT gt(String propId, Object value) {
		return filter(doc -> doc.hasProperty(propId) && doc.property(propId).compareTo(value) > 0);
	}
	...
	public RT notIn(String propId, String[] values) {
		return filter(doc -> doc.hasProperty(propId) && ArrayUtil.indexOf(values, doc.asString(propId)) < 0);
	}

	public RT contains(String propId, String[] values) {
		return filter(doc -> doc.hasProperty(propId) && ArrayUtil.contains(values, doc.asString(propId)));
	}

//	public RT hasRef(String refName, String targetPath) {
//		return filter(doc -> doc.hasRef(refName) && ArrayUtil.contains(doc.property(refName).asStrings(), targetPath));
//	}

	public RT startsWith(String propId, String prefix) {
		return filter(doc -> doc.hasProperty(propId) && doc.asString(propId, "").startsWith(prefix));
	}

	public RT endsWith(String propId, String suffix) {
		return filter(doc -> doc.hasProperty(propId) && doc.asString(propId, "").endsWith(suffix));
	}

	// Element
	public RT hasProperty(String... propIds) {
		return filter(doc ->  Arrays.asList(propIds).stream().map(p -> doc.hasProperty(p)).allMatch(Boolean.TRUE::equals) );
	}

	public RT type(String propId, Class clz) {
		return filter(doc -> doc.hasProperty(propId) && doc.property(propId) != null && clz.isInstance(doc.property(propId)));
	}

//	public RT size(String propId, int size) {
//		return filter(doc -> doc.property(propId).asStrings().length == size);
//	}
